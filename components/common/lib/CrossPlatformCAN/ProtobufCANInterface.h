...
// Message ID bit field constants
#define MSG_TYPE_SHIFT 15
#define COMP_TYPE_SHIFT 12
#define COMP_ID_SHIFT 9
#define CMD_ID_SHIFT 5
#define VALUE_TYPE_SHIFT 0

#define MSG_TYPE_MASK 0x1
#define COMP_TYPE_MASK 0x7
#define COMP_ID_MASK 0x7
...
public:
   ...
   * @param handler Function to call when matching message is received with just the value
   */
  ...
  /**
   * Helper function to pack a message ID
   */
  static uint16_t packMessageId(kart_common_MessageType msg_type,
  ...
  /**
   * Helper function to unpack a message ID
   */
  static void unpackMessageId(uint16_t message_id,
  ...
  /**
   * Helper function to get the length of a value type
   */
  static uint8_t getValueLength(kart_common_ValueType type);

  /**
   * Helper function to pack a value into a byte array
   */
  static void packValue(kart_common_ValueType type, int32_t value, uint8_t* data, uint8_t& length);

  /**
   * Helper function to unpack a value from a byte array
   */
