...
AHT21Sensor* tempAmbientSensor;
AHT21Sensor* humiditySensor;

// IMU Sensor (GY-521 / MPU-6050)
GY521Sensor* accelXSensor;
GY521Sensor* accelYSensor;
GY521Sensor* accelZSensor;
GY521Sensor* gyroXSensor;
GY521Sensor* gyroYSensor;
GY521Sensor* gyroZSensor;
GY521Sensor* imuTempSensor;

// GPS Sensor (ATGM336H)
// Note: We only need one instance to handle serial processing,
// but multiple logical sensor objects for reporting.
ATGM336HSensor* latitudeSensor;
ATGM336HSensor* longitudeSensor;
ATGM336HSensor* altitudeSensor;
ATGM336HSensor* speedSensor;
ATGM336HSensor* courseSensor;
ATGM336HSensor* satellitesSensor;
ATGM336HSensor* hdopSensor;
ATGM336HSensor* gpsFixSensor; // This one will also handle processSerial()

// Function Prototypes
void setupPins();
void setupSensors();
void handleImuConfigCommand(kart_common_MessageType msg_type, kart_common_ComponentType comp_type, uint8_t component_id, uint8_t command_id, kart_common_ValueType value_type, int32_t value);
void handleGpsConfigCommand(kart_common_MessageType msg_type, kart_common_ComponentType comp_type, uint8_t component_id, uint8_t command_id, kart_common_ValueType value_type, int32_t value);
#if DEBUG_MODE == 1
...
void setupSensors() {
  ...
  // --- Initialize ATGM336H Sensor Instances --- (Using Serial2)
  // IMPORTANT: Ensure Config.h defines GPS pins compatible with Serial2 and they don't conflict.
  //            If using Serial1, change &Serial2 to &Serial1 here.
  latitudeSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_LATITUDE, &Serial2, 1000);
  longitudeSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_LONGITUDE, &Serial2, 1000);
  altitudeSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_ALTITUDE, &Serial2, 1000);
  speedSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_SPEED, &Serial2, 1000);
  courseSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_COURSE, &Serial2, 1000);
  satellitesSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_SATELLITES, &Serial2, 1000);
  hdopSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_HDOP, &Serial2, 1000);
  gpsFixSensor = new ATGM336HSensor(kart_common_ComponentType_NAVIGATION, kart_navigation_NavigationComponentId_GPS_PRIMARY, kart_navigation_NavigationCommandId_GPS_FIX_STATUS, &Serial2, 1000);
  // Register ATGM336H
...
void parseSerialCommands() {
  if (Serial.available()) {
    ...
    if (commandUpper == "STATUS") {
      ...
      if (latitudeSensor) {
          TinyGPSPlus& gps = latitudeSensor->getGpsObject(); // Use getter
